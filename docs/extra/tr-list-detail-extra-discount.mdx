...
```html title="smarty"
    {if $P.NUMERIC1 && $P.IS_DISPLAY_PRODUCT == 0}
        ...
        {capture assign=PRICE} <strong class="text-primary"><span class="cart-price">{format price=($P.PRICE_SELL - ($P.PRICE_SELL * $P.NUMERIC1/100))}</span>{$P.CURRENCY}</strong>{/capture}
