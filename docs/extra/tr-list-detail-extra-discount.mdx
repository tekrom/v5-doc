...
window["extraDiscount"]=function(el){
    ...
    let discountPercent=parseFloat(item.querySelector("[data-extra]").getAttribute("data-extra"));
    let oldPrice=T.priceToFloat(item.querySelector(".product-price").innerText);
    let cartPrice=oldPrice-(oldPrice*discountPercent*0.01)
    item.querySelector(".cart-price").innerText=T.format(cartPrice)
}
