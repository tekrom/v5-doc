...
<div class="col-12">
    ...
    <div class="row">
        {foreach from=$SETTING.IMAGES item=IMAGE}
            ...
            <div class="col-12 col-lg-4 {*$IMAGE.COL.ALL} col-sm-{$IMAGE.COL.SM} col-md-{$IMAGE.COL.MD} col-lg-{$IMAGE.COL.LG} col-xl-{$IMAGE.COL.XL*} mb-2">
                <div href="{$IMAGE.URL}" class="w-100 d-block position-relative effect-wrapper map-point-wrapper" aria-label="{$IMAGE.TITLE}">
                    {if $IMAGE.IMG1.PATH || $IMAGE.IMG2.PATH}
                        <picture class="d-flex align-items-center justify-content-center overflow-hidden">
                            {if $IMAGE.IMG1.PATH}
                                <img {if $IS_LAZY_LOAD_ACTIVE == '1'} src="{$LAZY_LOAD_LOADING_IMAGE}" data-src="{$IMAGE.IMG1.PATH}"
                                    class="lazyload w-100 ease border-round" loading="lazy" {else} src="{$IMAGE.IMG1.PATH}" class="w-100 ease border-round" 
                                    {/if}
                        ...
                        {foreach from=$DATA_AREAS item=DATA key=key name=name}
                            ...
                            <span class="d-flex align-items-center justify-content-center banner-description position-absolute cursor-pointer border border-circle border-white map-point"
